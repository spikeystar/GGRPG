...
func _generate_collider():
	...
	for shape in [collision_body_shape, floor_notify_area_shape]:
		...
		var collider_edges = _get_collider_edges()
	...
	floor_notify_area.height = height
	
	collision_body.discover_shapes()

func _get_collider_edges():
	return {
		"left": Vector2(
...
func _generate_region_sprites():
	...
	var x_margin_width = (texture_size.x - (left_of_bottom_corner_width + right_of_bottom_corner_width)) / 2.0
	var top_margin = fmod(texture_size.y - HALF_TILE_HEIGHT - texture_offset.y, HALF_TILE_HEIGHT)
	
	var sprite_center = Vector2(
		x_margin_width + left_of_bottom_corner_width,
...
func _generate_collision_box_preview():
	...
	if Engine.editor_hint and preview_collision_box:
		if not collision_preview_mesh:
			collision_preview_mesh = MeshInstance2D.new()
		var array_mesh = ArrayMesh.new()
		var collider_edges = _get_collider_edges()
		var top_left = collider_edges.left + Vector2(0.0, COLLISION_PREVIEW_DRAW_OFFSET - height)
		var top_top = collider_edges.top + Vector2(0.0, COLLISION_PREVIEW_DRAW_OFFSET - height)
		var top_bottom = collider_edges.bottom + Vector2(0.0, COLLISION_PREVIEW_DRAW_OFFSET - height)
		var top_right = collider_edges.right + Vector2(0.0, COLLISION_PREVIEW_DRAW_OFFSET - height)
		...
		if collision_preview_mesh != null:
