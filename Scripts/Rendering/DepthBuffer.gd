...
const depth_buffer_object_transparent_shader = preload("res://Scripts/Rendering/DepthBufferObjectTransparent.gdshader")
...
func add_depth_test_mesh(
	...
	if depth_viewport:
		...
		if is_ready:
			...
			call_deferred("_set_mesh_global_translation", mesh_instance, Vector3(mesh_position.x , mesh_position.y, 0.0))
		mesh_instance.material_override = ShaderMaterial.new()
		if use_transparency:
			mesh_instance.material_override.shader = depth_buffer_object_transparent_shader
		else:
...
func add_depth_test_multimesh(
	...
	if depth_viewport:
		...
		for instance_index in range(0, instances.size()):
			var instance = instances[instance_index]
			multimesh.set_instance_transform(instance_index, Transform(Basis(), instance.position))
		
		var multimesh_instance = MultiMeshInstance.new()
		multimesh_instance.name = "DepthTestMultiMeshInstance"
		multimesh_instance.multimesh = multimesh
		multimesh_instance.material_override
		depth_root.add_child(multimesh_instance)
		if is_ready:
			multimesh_instance.global_translation = Vector3(mesh_position.x , mesh_position.y, 0.0)
		else:
			call_deferred("_set_mesh_global_translation", multimesh_instance, Vector3(mesh_position.x , mesh_position.y, 0.0))
		multimesh_instance.material_override = ShaderMaterial.new()
		if use_transparency:
			multimesh_instance.material_override.shader = depth_buffer_object_transparent_shader
		else:
			multimesh_instance.material_override.shader = depth_buffer_object_shader
		multimesh_instance.material_override.set_shader_param("depth_edge_buffer", DEPTH_EDGE_BUFFER)
		multimesh_instance.material_override.set_shader_param("sprite_texture_sampler", _create_texture_duplicate(texture))
		
		return weakref(multimesh_instance)
	else:
		return weakref(null)
...
func _set_mesh_global_translation(mesh, translation):
	mesh.global_translation = translation
