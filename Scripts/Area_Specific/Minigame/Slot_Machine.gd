...
func _input(event):
	if Input.is_action_just_pressed("ui_cancel") and wagering or Input.is_action_just_pressed("ui_accept") and wagering:
		done = false
		SE.effect("Menu Open")
...
func results():
	...
	if spade_count ==2 or diamond_count==2 or clover_count==2 or star_count==2:
		$Notify/Instruction.text = "Draw!"
		draw = true
		
	if spade_count <2 and diamond_count<2 and clover_count<2 and star_count<2:
		$Notify/Instruction.text = "Try Again!"
		Party.marbles -= (current_wager)
		fail = true
		
		
	yield(get_tree().create_timer(0.3), "timeout")	
	$Notify/AnimationPlayer.play("open")
	$Notify/AnimationPlayer2.play("flicker")
	$Notify.show()
	if win:
		SE.effect("Win")
	if draw:
